...
impl Entity {
    ...
    pub fn new_battery(node_id: u16, db_url: &str) -> Self {
        Self {
            ...
            query: format!(
                "SELECT CAST(battery AS DECIMAL(3, 2)) FROM measurements WHERE measurements.node = {node_id} ORDER BY \"when\" DESC LIMIT 1;"
            ),
            column: "battery",
            db_url: db_url.to_string(),
    ...
    pub fn new_wifi_essid(node_id: u16, db_url: &str) -> Self {
        Self {
            ...
            query: format!(
                "SELECT wifi_ssid FROM measurements WHERE measurements.node = {node_id} ORDER BY \"when\" DESC LIMIT 1;"
            ),
            column: "wifi_ssid",
            db_url: db_url.to_string(),
    ...
    pub fn new_wifi_rssi(node_id: u16, db_url: &str) -> Self {
        Self {
            ...
            query: format!(
                "SELECT wifi_rssi FROM measurements WHERE measurements.node = {node_id} ORDER BY \"when\" DESC LIMIT 1;"
            ),
            column: "wifi_rssi",
            db_url: db_url.to_string(),
